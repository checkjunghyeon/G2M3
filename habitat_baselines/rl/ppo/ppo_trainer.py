...
class PPOTrainerNO(BaseRLTrainerNonOracle):
    ...
    def _setup_actor_critic_agent(self, ppo_cfg: Config) -> None:
        ...
        self.actor_critic = BaselinePolicyNonOracle(
            ...
            global_map_semantic=self.config.RL.MAPS.global_map_semantic,
            global_map_goal=self.config.RL.MAPS.global_map_goal,
            coordinate_min=self.config.RL.MAPS.coordinate_min,
            coordinate_max=self.config.RL.MAPS.coordinate_max,
            aux_loss_seen_coef=ppo_cfg.aux_loss_seen_coef,
            aux_loss_direction_coef=ppo_cfg.aux_loss_direction_coef,
            aux_loss_distance_coef=ppo_cfg.aux_loss_distance_coef,
            configs=self.config
        )
    ...
    def _collect_rollout_step(
        ...
        with torch.no_grad():
            ...
            (
                ...
                rollouts.gt_semantic_map[rollouts.step],
        ...
        # final_rewards = [x+y for x,y in zip(rewards, acc_reward)]
        ...
        rewards = torch.tensor(
            final_rewards, dtype=torch.float, device=current_episode_reward.device
        )
    ...
    def _update_agent(self, ppo_cfg, rollouts):
        ...
        with torch.no_grad():
            ...
            next_value = self.actor_critic.get_value(
                last_observation,
                rollouts.semantic_segmentation[rollouts.step],
                rollouts.goal_image[rollouts.step],
                rollouts.recurrent_hidden_states[rollouts.step],
                rollouts.global_map[rollouts.step],
                rollouts.gt_semantic_map[rollouts.step],
                rollouts.prev_actions[rollouts.step],
    ...
    def train(self) -> None:
        ...
        rollouts = RolloutStorageNonOracle(
            ...
            self.config.RL.MAPS.global_map_depth + self.config.RL.MAPS.global_map_semantic + self.config.RL.MAPS.global_map_goal,  # 16
        )
        ...
        local_start_time_str = time.strftime(
            '%Y_%m_%d_%H_%M_%S', time.localtime(t_start)
        )
        env_time = 0
        pth_time = 0
        update = 0
        ...
        ckpt_path = self.config.CONTINUE_TRAINING

        if ckpt_path is not None:
            print("Loaded model is", ckpt_path)
            print(self.config.CONTINUE_TRAINING.split('_')[-8])
            print(self.config.CONTINUE_TRAINING.split('_')[-9])
            update = int(ckpt_path.split('_')[-8])  # for count_checkpoints
            count_steps = int(ckpt_path.split('_')[-9])  # for count_checkpoints
            ckpt_dict = torch.load(self.config.CONTINUE_TRAINING, map_location=lambda storage, loc: storage)
            self.agent.load_state_dict(ckpt_dict["state_dict"])

        lr_scheduler = LambdaLR(
            optimizer=self.agent.optimizer,
            lr_lambda=lambda x: linear_decay(update + 1, self.config.NUM_UPDATES),
        )  # x --> update + 1
        ...
        with TensorboardWriter(
            ...
            while update < self.config.NUM_UPDATES:
                update += 1
                ...
                total_found_actions = int(
                    torch.sum(rollouts.actions == 0).cpu().numpy()
                )
                total_forward_actions = int(
                    torch.sum(rollouts.actions == 1).cpu().numpy()
                )
                total_left_actions = int(torch.sum(rollouts.actions == 2).cpu().numpy())
                total_right_actions = int(
                    torch.sum(rollouts.actions == 3).cpu().numpy()
                )
                total_look_up_actions = int(
                    torch.sum(rollouts.actions == 4).cpu().numpy()
                )
                total_look_down_actions = int(
                    torch.sum(rollouts.actions == 5).cpu().numpy()
                )
                assert total_actions == (
                    total_found_actions
                    + total_forward_actions
                    + total_left_actions
                    + total_right_actions
                    + total_look_up_actions
                ...
                #     "map_encoder_cnn_0",
                #     self.actor_critic.net.map_encoder.cnn[0].weight,
                #     count_steps,
                # )
                # writer.add_histogram(
                #     "map_encoder_cnn_1",
                #     self.actor_critic.net.map_encoder.cnn[2].weight,
                #     count_steps,
                # )
                # writer.add_histogram(
                #     "map_encoder_cnn_2",
                #     self.actor_critic.net.map_encoder.cnn[4].weight,
                #     count_steps,
                # )
                # writer.add_histogram(
                #     "map_encoder_linear",
                #     self.actor_critic.net.map_encoder.cnn[6].weight,
                #     count_steps,
                # )

                writer.add_histogram(
                    "visual_encoder_cnn_0",
                    self.actor_critic.net.visual_encoder.cnn[0].weight,
                    count_steps,
                )
                writer.add_histogram(
                    "visual_encoder_cnn_1",
                    self.actor_critic.net.visual_encoder.cnn[2].weight,
                    count_steps,
                )
                writer.add_histogram(
                    "visual_encoder_cnn_2",
                    self.actor_critic.net.visual_encoder.cnn[4].weight,
                    count_steps,
                )
                writer.add_histogram(
                    "visual_encoder_linear",
                    self.actor_critic.net.image_features_linear.weight,
                    count_steps,
                )
                writer.add_scalar(
                    "train/found_action_prob",
                    total_found_actions / total_actions,
                    count_steps,
                )
                writer.add_scalar(
                    "train/forward_action_prob",
                    total_forward_actions / total_actions,
                    count_steps,
                )
                writer.add_scalar(
                    "train/left_action_prob",
                    total_left_actions / total_actions,
                    count_steps,
                )
                writer.add_scalar(
                    "train/right_action_prob",
                    total_right_actions / total_actions,
                    count_steps,
                )
                writer.add_scalar(
                    "train/look_up_action_prob",
                    total_look_up_actions / total_actions,
                    count_steps,
                )
                writer.add_scalar(
                    "train/look_down_action_prob",
                    total_look_down_actions / total_actions,
                    count_steps,
                )
                ...
                if len(metrics) > 0:
                    writer.add_scalar(
                        "metrics/distance_to_currgoal",
                        metrics["distance_to_currgoal"],
                        count_steps,
                    )
                    writer.add_scalar(
                        "metrics/success", metrics["success"], count_steps
                    )
                    writer.add_scalar(
                        "metrics/sub_success", metrics["sub_success"], count_steps
                    )
                    writer.add_scalar(
                        "metrics/episode_length", metrics["episode_length"], count_steps
                    )
                    writer.add_scalar(
                        "metrics/distance_to_multi_goal",
                        metrics["distance_to_multi_goal"],
                        count_steps,
                    )
                    writer.add_scalar(
                        "metrics/percentage_success",
                        metrics["percentage_success"],
                        count_steps,
                    )
                ...
                writer.add_scalar("train/loss_seen", loss_seen, count_steps)
                writer.add_scalar("train/loss_directions", loss_directions, count_steps)
                writer.add_scalar("train/loss_distances", loss_distances, count_steps)
                ...
                # checkpoint model - for continue_training
                #  print("self.config.CHECKPOINT_INTERVAL ", update, self.config.CHECKPOINT_INTERVAL )
                if update % self.config.CHECKPOINT_INTERVAL == 0:
                    self.save_checkpoint(
                        '{0}_{1}_{2}_{3}_{4}.pth'.format(
                            self.config.TRAINER_NAME, count_steps, update, local_start_time_str, count_checkpoints), dict(step=count_steps)
                    )
                    count_checkpoints += 1
                # toc = time.time()
                # print('## Update DONE (t={:0.2f}s).'.format(toc - tic))
            self.envs.close()
    ...
    def _eval_checkpoint(
        ...
        print("Loaded ckpt model is", checkpoint_path)
        ...
        # Removing auxiliary heads from ckpt dict (only useful at training time)
        keys_to_remove = []
        for key in ckpt_dict["state_dict"].keys():
            if "fc_seen" in key or "fc_direction" in key or "fc_distance" in key:
                keys_to_remove.append(key)
        for key in keys_to_remove:
            del ckpt_dict["state_dict"][key]
        ...
        test_global_map = torch.zeros(
            ...
            self.config.RL.MAPS.global_map_size,
            self.config.RL.MAPS.global_map_size,
            self.config.RL.MAPS.global_map_depth + self.config.RL.MAPS.global_map_semantic + self.config.RL.MAPS.global_map_goal,
        )
        ...
        nb_steps = np.zeros((self.config.NUM_PROCESSES))
        ...
        while (
            ...
            # cnt = 0
            # for env in current_episodes:
            #     print(cnt, ":", env.scene_id.split("/")[-1].split(".")[0])
            #     cnt += 1
        ...
        writer.add_scalar(
            "eval/distance_to_currgoal", metrics["distance_to_currgoal"], step_id
        )
        writer.add_scalar(
            "eval/distance_to_multi_goal", metrics["distance_to_multi_goal"], step_id
        )
        ...
        writer.add_scalar(
            "eval/percentage_success", metrics["percentage_success"], step_id
        )
...
def draw_projection(image, depth, s, global_map_size, coordinate_min, coordinate_max):
    ...
    spatial_locs, valid_inputs = _compute_spatial_locs(
        depth, s, global_map_size, coordinate_min, coordinate_max
    )
...
def _project_to_ground_plane(img_feats, spatial_locs, valid_inputs, s):
    ...
    idxes_ss = (
        (torch.arange(0, HbyK, 1) * K).long().to(device),
        (torch.arange(0, WbyK, 1) * K).long().to(device),
    )

    spatial_locs_ss = spatial_locs[
        :, :, idxes_ss[0][:, None], idxes_ss[1]
    ]  # (bs, 2, HbyK, WbyK)
    valid_inputs_ss = valid_inputs[
        :, :, idxes_ss[0][:, None], idxes_ss[1]
    ]  # (bs, 1, HbyK, WbyK)
    ...
    invalid_spatial_locs = (
        (spatial_locs_ss[:, 1] >= outh)
        | (spatial_locs_ss[:, 1] < 0)
        | (spatial_locs_ss[:, 0] >= outw)
        | (spatial_locs_ss[:, 0] < 0)
    )  # (bs, H, W)
...
def _compute_spatial_locs(
    depth_inputs, s, global_map_size, coordinate_min, coordinate_max
):
    bs, _, imh, imw = depth_inputs.shape
    local_scale = float(coordinate_max - coordinate_min) / float(global_map_size)
    cx, cy = 256.0 / 2.0, 256.0 / 2.0
    fx = fy = (256.0 / 2.0) / np.tan(np.deg2rad(79.0 / 2.0))
    # cx, cy = 128.0 / 2.0, 128.0 / 2.0
    # fx = fy = (128.0 / 2.0) / np.tan(np.deg2rad(79.0 / 2.0))
        ...
        while (
            ...
            for i in range(n_envs):
                ...
                # episode continues
                    ...
                    frame = observations_to_image(
                        observations[i], infos[i], actions[i].cpu().numpy()
                    )
                    rgb_frames[i].append(frame)
            (
                self.envs,
