...
class SEMSEGCNNNonOracle(nn.Module):
    r"""A Simple 3-Conv CNN followed by a fully connected layer
    ...
    def __init__(self, device):
        ...
        if self._n_input_semantic > 0:
            cnn_dims = np.array(
                (256, 256),  # observation_space.spaces["semantic"].shape[:2], (128, 128)
                dtype=np.float32
            )
        ...
        if self.is_blind:
            ...
            self.cnn = nn.Sequential(
                nn.Conv2d(
                    in_channels=self._n_input_semantic,
                    out_channels=32,
                    kernel_size=self._cnn_layers_kernel_size[0],
                    stride=self._cnn_layers_stride[0],
                ),
    ...
    @property
    def is_blind(self):
        return self._n_input_semantic == 0
    ...
    def forward(self, semantic_image):
        ...
        if self._n_input_semantic > 0:
            # permute tensor to dimension [BATCH x CHANNEL x HEIGHT X WIDTH]
            semantic_observations = semantic_image
            cnn_input.append(semantic_observations)

        cnn_input = torch.cat(cnn_input, dim=1)

        return self.cnn(cnn_input.to(self.device))


class GOALCNNNonOracle(nn.Module):
    r"""A Simple 3-Conv CNN followed by a fully connected layer
    ...
    def __init__(self, device):
        ...
        if self._n_input_semantic > 0:
            cnn_dims = np.array(
                (128, 128), # observation_space.spaces["semantic"].shape[:2], (128, 128)
                dtype=np.float32
            )
    ...
    def forward(self, goal_image):
        cnn_input = []

        if self._n_input_semantic > 0:
            # permute tensor to dimension [BATCH x CHANNEL x HEIGHT X WIDTH]
            goal_observations = goal_image
...
class MapCNN(nn.Module):
    ...
    def __init__(self, map_size, output_size, agent_type):
        ...
        if agent_type == "oracle-ego":
            self._n_input_map = 16
        elif agent_type == "non-oracle":  ## added for semseg
            self._n_input_map = 16  # 32 + 17 + 8
        else:
