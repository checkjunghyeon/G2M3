...
def get_grid(pose, grid_size, device):
    ...
    Input:
        ...
        `device` torch.device (cpgu or gpu)
    Output:
...
class ComputeSpatialLocs:
    def __init__(
        ...
        self.device = device
        # for 256x256
        # self.cx, self.cy = 256.0 / 2.0, 256.0 / 2.0  # Hard coded camera parameters
        # self.fx = self.fy = (256.0 / 2.0) / np.tan(np.deg2rad(79 / 2.0))
        # for 128x128
        self.cx, self.cy = 128.0 / 2.0, 128.0 / 2.0  # Hard coded camera parameters
        self.fx = self.fy = (128.0 / 2.0) / np.tan(np.deg2rad(79 / 2.0))
        self.egocentric_map_size = egocentric_map_size
        self.local_scale = float(coordinate_max - coordinate_min) / float(
            global_map_size
        )
    ...
    def forward(self, depth):
        ...
        x_gp = (
            ((X / self.local_scale) + (self.egocentric_map_size - 1) / 2).round().long()
        )  # (bs, imh, imw, 1)
        y_gp = (
            (-(Z / self.local_scale) + (self.egocentric_map_size - 1) / 2)
            .round()
            .long()
        )  # (bs, imh, imw, 1)
...
class ProjectToGroundPlane:
    ...
    def forward(self, conv, spatial_locs, valid_inputs):
        ...
        # for 128x128
        # K = 128 / 12  # Hardcoded value of K
        K = 1

        # Sub-sample spatial_locs, valid_inputs according to img_feats resolution.
        idxes_ss = (
            (torch.arange(0, HbyK, 1) * K).long().to(self.device),
        ...
        linear_locs_ss = (
            spatial_locs_ss[:, 1] * outw + spatial_locs_ss[:, 0]
        )  # (bs, H, W)
