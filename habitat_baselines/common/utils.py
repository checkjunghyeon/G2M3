...
def batch_obs(
    ...
    for obs in observations:
        for sensor in obs:
            if sensor == 'semantic':
                batch[sensor].append(_to_tensor(obs[sensor].astype('int64')).unsqueeze(-1))
            else:
                batch[sensor].append(_to_tensor(obs[sensor]))
...
def poll_checkpoint_folder(
    ...
    models_paths.sort(key=lambda x: int(x.split(".")[0].split("/")[-1].split("_")[1]))
...
class Model(nn.Module):
    ...
    def get_latest_model(self, save_dir):
        ...
        latest_checkpoint = None if (len(checkpoint_list) is 0) else \
        checkpoint_list[-1]
        return latest_checkpoint

    def load_model(self, models, checkpoint_file):
        ...
        for model in models:
            if model in checkpoint['models']:
                ...
                raise Exception(
                    "Missing model in checkpoint: {}".format(model))
        return models